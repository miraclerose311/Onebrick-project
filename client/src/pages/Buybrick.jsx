...
const Buybrick = () => {
	...
	const [randomClickedBrickId, setRandomClickedBrickId] = useState("");
	...
	const setBuybrickModalInCenter = () => {
		...
		const brickId =
			bricks[Math.floor(Math.random() * CurrentBricks.length)].brick_id;
		console.log("CLicked brickId", brickId);
		setRandomClickedBrickId(brickId);
		handleSetIsBuyBrickModalOpen(true);
		...
		<div className='text-center items-center h-screen bg-white w-full flex flex-col itmes-center sm:justify-center'>
			...
			{isBuyBrickModalOpen && (
				<MemorizedBuybrickModal
					...
					clickedBrick={randomClickedBrickId}
